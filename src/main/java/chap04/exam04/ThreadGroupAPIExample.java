...
public class ThreadGroupAPIExample {
    public static void main(String[] args) throws InterruptedException {
       ...
       /* for (Thread thread : topThreads) {
        ...
        log.info("상위그룹 활성 스레드 수: {}", topGroup.activeCount());
        log.info("상위그룹 활성 스레드 그룹 수: {}", topGroup.activeGroupCount());
        log.info("하위 그룹 활성 스레드 수 : {}", subGroup.activeCount());
        log.info("하위 그룹 활성 스레드 그룹 수 : {}", subGroup.activeGroupCount());

        log.info("하위그룹의 부모 그룹: {}", subGroup.getParent().getName());

        log.info("상위그룹은 하위그룹의 부모 또는 조상 그룹인가? {}", topGroup.parentOf(subGroup));
